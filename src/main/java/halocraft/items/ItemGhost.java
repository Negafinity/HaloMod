...
public class ItemGhost extends Item {
	...
	public static final ItemGhost instance = new ItemGhost();
	public static final String name = "itemGhost";
	public ItemGhost()
	{
		this.maxStackSize = 1;
		...
		else
		{
			...
			else
			{
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
				{
					...
					EntityGhost entityghost = new EntityGhost(worldIn, (double)((float)blockpos.getX() + 0.5F), (double)((float)blockpos.getY() + 1.0F), (double)((float)blockpos.getZ() + 0.5F));
					entityghost.rotationYaw = (float)(((MathHelper.floor_double((double)(playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

					if (!worldIn.getCollidingBoundingBoxes(entityghost, entityghost.getEntityBoundingBox().expand(-0.1D, -0.1D, -0.1D)).isEmpty())
					{
						return itemStackIn;
					}
					...
					if (!worldIn.isRemote)
					{
						worldIn.spawnEntityInWorld(entityghost);
					}
