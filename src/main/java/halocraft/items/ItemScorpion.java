...
public class ItemScorpion extends Item
{
	// Following is so you can access it in pre-init
	public static final ItemScorpion instance = new ItemScorpion();
	public static final String name = "itemScorpion";
		...
		else
		{
			...
			else
			{
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
				{
					...
					EntityScorpion entityscorpion = new EntityScorpion(worldIn, (double) ((float) blockpos.getX() + 0.5F), (double) ((float) blockpos.getY() + 1.0F), (double) ((float) blockpos.getZ() + 0.5F));
					entityscorpion.rotationYaw = (float) (((MathHelper.floor_double((double) (playerIn.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);
					...
					if (!worldIn.isRemote)
					{
						worldIn.spawnEntityInWorld(entityscorpion);
					}
