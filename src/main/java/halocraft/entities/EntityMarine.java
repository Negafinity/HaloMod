...
public class EntityMarine extends EntityCreature implements IMob, IRangedAttackMob
{
	public EntityMarine(World worldIn)
	{
		...
		this.setCurrentItemOrArmor(0, new ItemStack(ItemAssaultRifle.instance));
	...
	{
		return new ItemStack(ItemAssaultRifle.instance, 1);
	}
	...
	{
		if (entityLivingIn instanceof EntityMob)
		{
			EntityBullet bullet = new EntityBullet(this.worldObj, this);
			...
			if (this.getAttackTarget() != null)
			{
				...
				if (d3 >= 1.0E-7D)
				{
					...
					double d4 = d0 / d3;
					double d5 = d2 / d3;
					bullet.setLocationAndAngles(this.posX + d4, this.posY, this.posZ + d5, f2, f3);
					bullet.setThrowableHeading(d0, d1, d2, 1, 0.25f);
				}
