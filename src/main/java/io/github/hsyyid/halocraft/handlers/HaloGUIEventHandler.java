...
public class HaloGUIEventHandler extends Gui
{
	...
	protected void drawQuad(int x, int y, int w, int h, float minU, float minV, float maxU, float maxV)
	{
		...
		vertexBuffer.begin(7, DefaultVertexFormats.POSITION_TEX); // StartDrawingQuads
		vertexBuffer.pos(x + 0, y + h, (double) zLevel).tex(minU, maxV).endVertex();
		vertexBuffer.pos(x + w, y + h, (double) zLevel).tex(maxU, maxV).endVertex();
		vertexBuffer.pos(x + w, y + 0, (double) zLevel).tex(maxU, minV).endVertex();
		vertexBuffer.pos(x + 0, y + 0, (double) zLevel).tex(minU, minV).endVertex();
