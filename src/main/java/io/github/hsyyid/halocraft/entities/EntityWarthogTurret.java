...
public class EntityWarthogTurret extends Entity
{
	private boolean shouldRenderRider;
	...
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(EntityGhost.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(EntityGhost.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(EntityGhost.class, DataSerializers.FLOAT);

	public EntityWarthogTurret(World worldIn)
	{
		super(worldIn);
	...
	public EntityWarthogTurret(World worldIn, double posX, double posY, double posZ)
	{
		this(worldIn);
	...
	public boolean attackEntityFrom(DamageSource source, float amount)
	{
		...
		{
			if (this.getControllingPassenger() != null && this.getControllingPassenger() == source.getEntity() && source instanceof EntityDamageSourceIndirect)
			{
				return false;
			}
			else
	...
	{
		if (p_180426_10_ && this.getControllingPassenger() != null)
		{
	...
	public void onUpdate()
	{
		...
		for (int i = 0; i < b0; ++i)
		{
			double d1 = this.getEntityBoundingBox().minY + (this.getEntityBoundingBox().maxY - this.getEntityBoundingBox().minY) * (double) (i) / (double) b0 - 0.125D;
		...
		else
		{
			...
			if (!this.worldObj.isRemote)
			{
				...
				{
					list.stream().filter(entity -> entity != this.getControllingPassenger() && entity.canBePushed() && entity instanceof EntityGhost).forEach(entity -> entity.applyEntityCollision(this));
				}
	...
	public void updateRiderPosition()
	{
		if (this.getControllingPassenger() != null)
		{
			...
			this.getControllingPassenger().setPosition(this.posX + d0, this.posY + this.getMountedYOffset() + this.getControllingPassenger().getYOffset(), this.posZ + d1);
		}
		if (worldObj.isRemote)
		{
			if (this.getControllingPassenger() instanceof EntityPlayer)
			{
				if (Minecraft.getMinecraft().thePlayer == this.getControllingPassenger() && Minecraft.getMinecraft().gameSettings.thirdPersonView != 1)
				{
	...
	protected void func_180433_a(double p_180433_1_, boolean p_180433_3_, Block p_180433_4_, BlockPos p_180433_5_)
	{
		...
		else if (this.worldObj.getBlockState((new BlockPos(this)).down()).getBlock().getMaterial(null) != Material.water && p_180433_1_ < 0.0D)
		{
	...
	{
		this.dataWatcher.set(DAMAGE_TAKEN, damage);
	}
	...
	{
		this.dataWatcher.set(TIME_SINCE_HIT, time);
	}
	...
	{
		return shouldRenderRider;
	}

	public void setShouldRenderRider(boolean shouldRenderRider)
	{
		this.shouldRenderRider = shouldRenderRider;
	}
}
